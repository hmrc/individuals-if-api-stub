...
class IncomePayeRepository  @Inject()(mongoConnectionProvider: MongoConnectionProvider)(implicit val ec: ExecutionContext)
  ...
  def findByTypeAndId(idType: String,
    ...
    val fieldsMap = Map(
      ...
      "paye(employee(hasPartner),employeePensionContribs(paidYTD,notPaidYTD,paid,notPaid),grossEarningsForNICs(inPayPeriod1,inPayPeriod2,inPayPeriod3,inPayPeriod4),payFrequency,paymentDate,taxablePay)" -> "HMCTS-C2_HMCTS-C3",
