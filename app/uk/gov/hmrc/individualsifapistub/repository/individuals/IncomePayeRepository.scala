...
class IncomePayeRepository  @Inject()(mongoConnectionProvider: MongoConnectionProvider)(implicit val ec: ExecutionContext)
  ...
  def findByTypeAndId(idType: String,
    ...
    val fieldsMap = Map(
      ...
      "paye(employeePensionContribs(paidYTD,notPaidYTD,paid,notPaid),taxablePay,employee(hasPartner),grossEarningsForNICs(inPayPeriod1,inPayPeriod2,inPayPeriod3,inPayPeriod4),payFrequency,paymentDate)" -> "HMCTS-C2_HMCTS-C3",
